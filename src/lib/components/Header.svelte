...
<header class="site-header" class:header-hidden={hidden}>
	<div class="site-header-container">
		...
		<nav class="nav-desktop">
			...
			<button 
				type="button"
				class="button-primary header-cta-responsive"
				onclick={() => window.location.href = '/beta-2/apply'}
				aria-label="Apply for Beta 2 program"
			>
				<span class="cta-text-mobile">Join Beta 2</span>
				<span class="cta-text-desktop">Apply for Beta 2</span>
			</button>
					</div>
				{/if}
...
<style>
	/* Site Header - Refined & Professional */
	.site-header {
		position: fixed;
		top: 0;
		left: 0;
	...
	/* Header CTA - Responsive sizing for compact header */
	...
	.mobile-logout-button:hover:not(:disabled) {
		color: var(--text-primary);
		background: var(--bg-secondary);
		border-color: var(--border-secondary);
		transform: translateY(-1px);

...
export const load: PageServerLoad = async ({ locals, url, params, parent }) => {
	...
	try {
		...
		// Load tour data with ownership verification - simple query
		const tour = await loadTourWithOwnership(tourId, userId);

		// Get only upcoming time slots to reduce load - limit to 50
		const now = new Date();
		...
		const processedTimeSlots = tourTimeSlots.map(slot => ({
			...slot,
		...
		// Return safe fallback data instead of throwing 500
		try {
			const [tour] = await db
				.select()
				.from(tours)
			...
			return {
				...
				timeSlots: [] // Return empty array instead of crashing
			};
		} catch (fallbackError) {
	...
	'delete-slot': async ({ request, locals, params }) => {
		...
		try {
			...
			if (slot.bookedSpots > 0) {
				return { success: false, error: 'Cannot delete time slot with existing bookings' };
			}

-- Migration: Add Cross-Border Payment Support for pgweb
-- Copy this entire script and paste into pgweb Query tab, then execute
...
-- New payment indexes (using regular CREATE INDEX for transaction compatibility)
CREATE INDEX IF NOT EXISTS idx_payments_payment_type ON payments(payment_type);
CREATE INDEX IF NOT EXISTS idx_payments_tour_guide_user_id ON payments(tour_guide_user_id);
CREATE INDEX IF NOT EXISTS idx_payments_payout_id ON payments(payout_id);
CREATE INDEX IF NOT EXISTS idx_payments_payout_completed ON payments(payout_completed);

-- Payout indexes
CREATE INDEX IF NOT EXISTS idx_payouts_tour_guide_user_id ON payouts(tour_guide_user_id);
CREATE INDEX IF NOT EXISTS idx_payouts_status ON payouts(status);
CREATE INDEX IF NOT EXISTS idx_payouts_stripe_payout_id ON payouts(stripe_payout_id);
CREATE INDEX IF NOT EXISTS idx_payouts_period_start ON payouts(period_start);
...
CREATE INDEX IF NOT EXISTS idx_payout_items_payment_id ON payout_items(payment_id);
CREATE INDEX IF NOT EXISTS idx_payout_items_created_at ON payout_items(created_at);
...
-- =====================================================
-- STEP 6: Add triggers for updated_at (ONLY if function exists)
-- =====================================================
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_updated_at_column') THEN
        -- Try to create trigger only if function exists
        BEGIN
            CREATE TRIGGER update_payouts_updated_at BEFORE UPDATE ON payouts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
            RAISE NOTICE 'Trigger update_payouts_updated_at created successfully';
        EXCEPTION
            WHEN duplicate_object THEN 
    ...
    ELSE
        ...
        RAISE NOTICE 'updated_at column will not auto-update for payouts table';
    END IF;
END $$;
...
WHERE trigger_name = 'update_payouts_updated_at';
